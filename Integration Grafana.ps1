...
$DOWLOAD_URL = httpsgithub.comgrafanaagentreleaseslatestdownloadgrafana-agent-installer.exe.zip
...
$CONFIG_URI = $GCLOUD_API_URLstacks$GCLOUD_STACK_IDagent_configplatforms=windows
  ...
  Set-Content -Path $config_file -Value ($yamlConfig)
    ...
    if ($line -ne $null) {
        ...
        $line  ForEach-Object {
            ...
            $content = $content -replace $_,$($prefix) $($envAPPDATA)$($bookmark_filename)
        }
        $content  Set-Content $config_file
    }
    Move-Item $config_file CProgram FilesGrafana Agentagent-config.yaml -force
